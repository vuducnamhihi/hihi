import { PrismaClient, Role, RoomStatus, ContractStatus, InvoiceStatus } from '@prisma/client';
...
async function main() {
  ...
  // 5. Tạo Hợp đồng đang thuê (ACTIVE)
  ...
  // 6. Tạo Hóa đơn mẫu với nhiều trạng thái để test luồng
  // Hóa đơn 1: Đã phát hành (PENDING_PAYMENT) cho Trần Thị Thuê Nhà
  await prisma.invoice.create({
    data: {
      ...
      waterUsage: 5,
      waterAmount: 5 * 28000,
      otherFees: 100000, // Tiền rác & wifi
      totalAmount: 3500000 + (110 * 3800) + (5 * 28000) + 100000, // 4,158,000
      status: InvoiceStatus.PENDING_PAYMENT,
      dueDate: new Date('2026-08-05'),
    },
  });

  // Hóa đơn 2: Khách đã nộp ảnh bill (PAYMENT_SUBMITTED) cho Lê Văn An
  await prisma.invoice.create({
    data: {
  ...
  // Hóa đơn 3: Hóa đơn DRAFT do BullMQ tạo tự động cho tháng tiếp theo
  await prisma.invoice.create({
    data: {
      contractId: contract1.id,
      roomId: room101.id,
